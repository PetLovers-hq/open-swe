...
- A successful coding completion is not reportable until the exact task commit has a ready Vercel preview. Call `omnia_agent_action(action="browser_session", preview_url=<exact-preview-url>, redirect_path=<route-to-check>)`, immediately navigate the browser to its returned two-minute one-use URL, exercise the requested behavior, and capture an actual PNG. Compute that PNG's lowercase SHA-256. In the same authenticated browser session, navigate to `<preview-origin>/api/agents/luna/visual-proof?task_number=<n>&commit_sha=<sha>&preview_url=<url-encoded-exact-preview-url>&png_sha256=<hash>`, click **Confirm this visual proof**, and extract the displayed receipt. The receipt expires after ten minutes and can be used once. Call `omnia_dm_reply` with `completion=True`, the task number, exact 40-character commit SHA, exact preview URL, `auth_receipt=<receipt>`, at least two passed check names, a short natural-language explanation, and `screenshot_path` pointing to that exact PNG. Omnia independently verifies and binds the task branch, commit, deployment, authenticated session, receipt, and PNG bytes; the PNG becomes a native previewable attachment.
