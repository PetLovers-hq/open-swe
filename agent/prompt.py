...
- A successful coding completion is not reportable until the exact task commit has a ready Vercel preview. Call `omnia_agent_action(action="browser_session", preview_url=<exact-preview-url>, redirect_path=<route-to-check>)`. Immediately run the durable sandbox proof driver from `origin/main`: copy `scripts/luna-visual-proof.mts`, `scripts/lib/tieout.mts`, and `scripts/lib/chromeProfile.mts` from `origin/main` into `.luna-evidence/proof-runtime/scripts/` with the same relative layout, then execute `node --experimental-transform-types .luna-evidence/proof-runtime/scripts/luna-visual-proof.mts <one-use-browser-session-url> <absolute-png-path> <task-number> <exact-40-character-commit> <exact-preview-url>`. Do not use HTTP requests for this flow and do not delegate it to the hosted browser subagent: the proof driver launches Chromium inside the durable coding sandbox, consumes the authenticated bootstrap, captures the real PNG, and confirms the PNG-bound receipt in one browser session. It prints structured JSON containing `screenshot_path`, `png_sha256`, `receipt`, and rendered page evidence. The receipt expires after ten minutes and can be used once. Call `omnia_dm_reply` with `completion=True`, the task number, exact 40-character commit SHA, exact preview URL, `auth_receipt=<receipt>`, at least two passed check names, a short natural-language explanation, and `screenshot_path` set to the exact path printed by the proof driver. Omnia independently verifies and binds the task branch, commit, deployment, authenticated session, receipt, and PNG bytes; the PNG becomes a native previewable attachment.
