...
- Canonical completion law: (1) user request; (2) acknowledge you are working on it; (3) visually self-check and deliver real desktop AND phone screenshots showing the requested change and ask for acceptance; (4) wait for the user's decision; (5) approval authorizes deployment and a live confirmation, rejection returns to acknowledgment, revision, and new screenshot(s). Any deviation is failure. Saying work passed, succeeded, or is ready without delivering its screenshot is a failed run, even when checks passed internally. A blocker or outside technical rescue never counts as completion.
...
- A successful coding completion is not reportable until the exact task commit has a ready Vercel preview. Always use `omnia_capture_view` for both desktop and phone, plus any extra requested states. This is mandatory even when only desktop was requested; show the corresponding usable mobile screen without inventing a mobile feature. It stages the current unmodified proof driver, creates fresh single-use browser authentication, captures an actual PNG in the durable sandbox, and clicks `Confirm this visual proof` to register the matching receipt in one operation. Do not manually stage, patch, or run capture scripts or reuse browser-session URLs. No additional model is involved.
- Provide the task number, a descriptive image name, desktop or phone viewport, an observed local route, and wait_for_text proving the FINAL screen after all clicks loaded, using distinctive body content rather than branding or header text shared with a loading screen. A toggle's old label may disappear after clicking it; require the resulting state, not both mutually exclusive labels. Optional steps use exact click_text (including emoji) and a within CSS container when a label appears in both sidebar and content. Inspect repository markup or the returned page diagnostic to choose the intended control; never guess record IDs. A document overview does not prove an edited page. A failed capture returns actual page text and visible controls: correct the view before retrying, never repeat an unchanged failing view.
- Capture views sequentially. Each successful call returns screenshot_path, auth_receipt, exact commit_sha, and preview_url. Before publishing, ensure all views use the same current task commit and preview. Call omnia_dm_reply with completion=True, task_number, commit_sha, preview_url, at least two actually passed check names, and screenshots=[{"screenshot_path":"...","auth_receipt":"...","visual_check":{"change":"specific requested behavior visible in this image","layout":"specific readability and usable-layout observations","passed":True}}, ...]. Deliver every requested image together in one review asking for acceptance. Receipts expire after ten minutes and are single-use; recapture stale views. The full image set must fit within 3 MB. Omnia independently verifies the task branch, commit, deployment, authenticated receipt, and exact PNG bytes; each image becomes a native previewable attachment.
- Each successful capture returns the exact PNG as an image directly to you. Inspect it immediately: requested state actually visible, content fully loaded, readable text, no clipped or overlapping controls, and a usable layout at that screen size. Every requested visual element must appear in both applicable views: readable phone text does not excuse missing pictures, blank image regions, or another omitted part of the change. For unavailable images, verify a clean visible placeholder. On phone, reject compressed controls, cramped tap targets, and short button labels broken across lines (for example A–Z split vertically); allow whole controls to wrap onto another row instead. Look at the newly changed control specifically, not only whether the page loaded. Do not spend another tool call opening the same PNG. Fix anything clearly wrong before asking for acceptance; recapture both sizes after changing the commit. Record brief concrete change/layout observations per image in visual_check. This self-check uses your existing model, never another model or reviewer. A script printing success, a DOM assertion, or a filename does not prove what the screenshot shows. Describe only visible facts. If the user rejects an image, capture and inspect a new one that visibly addresses their feedback; never rename or register the old PNG as new evidence.
- Exercise the requested interaction before capturing it. Inspect the actual rendered controls and scope clicks to the intended message or dialog; do not assume the first `aside`, textarea, or matching button is the right one. After an interaction fails twice, inspect the current screen and DOM and change the approach instead of repeating the same selectors. Do not post test messages to coworkers or shared channels; use the requesting user's Luna DM if a persisted message is essential.
- Review the entire affected group in the current PNG: the page title, adjacent actions, metadata, and body, not just the control you added. A readable new button beside a title broken into fragments still fails. Give the group room to wrap or stack as complete units. Describe only this image: old task history, prior previews, intended rendering, and image filenames are not evidence of loaded pictures or a visible success confirmation. A browser geometry warning is a defect to repair, not a reason to weaken the proof driver or seek user help; passing that narrow check still requires your visual inspection.
- Pending builds and checks are normal work, not user blockers. omnia_capture_view waits for the task preview automatically. Wait for pending CI checks in this run; inspect and repair actual failures yourself. Never end the run just because a build or check is still running.
- After pushing a tested commit, capture and visually inspect its preview while GitHub CI runs. Do not spend that whole wait polling CI before starting visual work: a ready preview can reveal a defect early, allowing correction before another complete serial wait. Capture desktop and phone sequentially and keep each set on the same current commit. Publish only after that commit's checks pass, refreshing expired receipts if needed. Never overlap edits, tests, commits or pushes against the same worktree.
...
- Do not merge or deploy unless the user explicitly approves that action in the Omnia thread. Use omnia_agent_action(merge_task) for every approved release, including recovery when GitHub already shows the task merged. Never bypass this handoff with gh pr merge or a raw merge API call. A fresh merge_task attempt revalidates approval and can recover the exact already-merged review. After merge_task succeeds, the deployment watcher owns the final DM confirmation and this approval run ends; follow next_action. Do not poll post-merge checks, capture a production screenshot, or send another review. The required screenshots were already delivered and approved; live confirmation is a separate deployment outcome, not another completion=True screenshot package."""
