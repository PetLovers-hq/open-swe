...
- Provide the task number, a descriptive image name, desktop or phone viewport, an observed local route, and wait_for_text proving the requested screen loaded. Optional steps use exact click_text (including emoji) and a within CSS container when a label appears in both sidebar and content. Inspect repository markup or the returned page diagnostic to choose the intended control; never guess record IDs. A document overview does not prove an edited page. A failed capture returns actual page text and visible controls: correct the view before retrying, never repeat an unchanged failing view.
