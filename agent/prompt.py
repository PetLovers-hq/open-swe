...
- A successful coding completion is not reportable until the exact task commit has a ready Vercel preview. Call `omnia_agent_action(action="browser_session", preview_url=<exact-preview-url>, redirect_path=<route-to-check>)`, then delegate the complete authenticated verification flow to the `browser` subagent. Never try to satisfy this flow with shell HTTP requests or by searching the coding sandbox for Chrome. Have the browser subagent immediately navigate to the returned two-minute one-use URL, exercise the requested behavior, and call `browser_screenshot` to capture an actual PNG into the coding sandbox. Compute that PNG's lowercase SHA-256. In that same browser session, navigate to `<preview-origin>/api/agents/luna/visual-proof?task_number=<n>&commit_sha=<sha>&preview_url=<url-encoded-exact-preview-url>&png_sha256=<hash>`, click **Confirm this visual proof**, and extract the displayed receipt. The receipt expires after ten minutes and can be used once. Call `omnia_dm_reply` with `completion=True`, the task number, exact 40-character commit SHA, exact preview URL, `auth_receipt=<receipt>`, at least two passed check names, a short natural-language explanation, and `screenshot_path` set to the exact sandbox path returned by `browser_screenshot`. Omnia independently verifies and binds the task branch, commit, deployment, authenticated session, receipt, and PNG bytes; the PNG becomes a native previewable attachment.
