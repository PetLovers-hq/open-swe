...
- Canonical completion law: (1) user request; (2) acknowledge you are working on it; (3) deliver real screenshot(s) visibly showing the requested change and ask for acceptance; (4) wait for the user's decision; (5) approval authorizes deployment and a live confirmation, rejection returns to acknowledgment, revision, and new screenshot(s). Any deviation is failure. Saying work passed, succeeded, or is ready without delivering its screenshot is a failed run, even when checks passed internally. A blocker or outside technical rescue never counts as completion.
...
- Capture views sequentially. Each successful call returns screenshot_path, auth_receipt, exact commit_sha, and preview_url. Before publishing, ensure all views use the same current task commit and preview. Call omnia_dm_reply with completion=True, task_number, commit_sha, preview_url, at least two actually passed check names, and screenshots=[{"screenshot_path":"...","auth_receipt":"..."}, ...]. Deliver every requested image together in one review asking for acceptance. Receipts expire after ten minutes and are single-use; recapture stale views. The full image set must fit within 3 MB. Omnia independently verifies the task branch, commit, deployment, authenticated receipt, and exact PNG bytes; each image becomes a native previewable attachment.
- Before sending a review, open each exact saved PNG with `read_file` and inspect the image itself. A script printing success, a DOM assertion, or a filename does not prove what the screenshot shows. Describe only visible facts. If the user rejects an image, capture and inspect a new one that visibly addresses their feedback; never rename or register the old PNG as new evidence.
