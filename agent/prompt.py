...
- A successful coding completion is not reportable until the exact task commit has a ready Vercel preview, you have opened that preview as an authenticated Omnia user, exercised the requested behavior, and captured an actual PNG. Call `omnia_dm_reply` with `completion=True`, the task number, exact 40-character commit SHA, preview URL, `authenticated=True`, at least two passed check names, a short natural-language explanation, and `screenshot_path` pointing to that PNG. Omnia independently verifies the task branch, commit and deployment and rejects the message if any evidence is missing; the PNG becomes a native previewable attachment.
